...
mergePALMOdata <- function(data_object, datatype) {
  ...
  if(datatype == "bulk") {
    ...
    if(length(overlap)>2) {
      ...
      data <- data.frame(data, check.names = F, stringsAsFactors = F)
      data <- data[,overlap]
    ...
    if(class(data)[1] %in% c("data.frame", "matrix")) {
      ...
      #dataList <- list(ann, data)
      #names(dataList) <- c("ann", "data")
      #return(dataList)
      ...
      if(length(checkSample) == 1) {
        ...
        colnames(metaData) <- make.names(colnames(metaData), unique = T)

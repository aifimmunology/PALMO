...
#' @param ann Annotation dataframe. Table must consist column Sample (Participant
#' sample name), PTID (Participant), Time (longitudinal time points)
#' @param dataObj Single cell RNA seurat object. Seurat object should have column
#' name Sample (same as annotation table Sample column)
#' @param scassay Single cell assay from scRNA seurat object (Default "RNA")
#' @param celltypecol Column of interest such as celltype to analyze DEGs in
#' participant over time
#' @param mincellsexpressed Average expression threshold to filter lowly expressed
...
sclongitudinalDEG <- function(ann, dataObj, scassay="RNA", celltypecol,
           mincellsexpressed = 0.1, removelnc = "TRUE", adjfac = "none",
           baseline = NULL,
           plotWidth=10, plotHeight=10, fileName=NULL, filePATH=NULL) {
    ...
    #colnames(scmetadata)[which(colnames(scmetadata) == "batch")] <- "Sample"
    ## xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx

    # merge metadata in input CSV with sc object metadata
    scmetadata <- merge(scmetadata, ann, by = "Sample", all.x = TRUE) %>%
          column_to_rownames(var = "barcodes")
    scmetadata <- scmetadata[colnames(sccounts), , drop = F]
    #table(rownames(scmetadata) == colnames(sccounts))

    # fetch PTIDs and celltypes
    ptids <- sort(as.character(unique(scmetadata[[donorIDcol]])))
    ptids <- ptids[!is.na(ptids)]
    celltypes <- sort(as.character(unique(scmetadata[[celltypecol]])))
    ...
    lmLS <- lapply(ptids, function(ptid) {
      ...
      cat(date(),": Fitting a ZLM model for donorID: ", ptid,"\n")

      #Consider basline
      if(baseline==TRUE) {
        cat(date(),": Performing baseline analysis\n")
      ...
      sccounts_sub <- sccounts[, rownames(scmetadata_sub), drop = F]

      # initiating loop for per cell type within a donor analysis
      lapply(celltypes, function(ct) {
        print(paste("Fitting a ZLM model for donorID: ", ptid,
            ", within cell type: ", ct, sep = ""))

         # subset counts on selected cell type barcodes
         selcells <- subset(scmetadata_sub, scmetadata_sub[[celltypecol]] == ct)
         selcells <- rownames(selcells)
         sccounts_sub_ct <- sccounts_sub[, selcells, drop = F]
        ...
        scmetadata_sub <- scmetadata_sub[colnames(sccounts_sub_ct), , drop = F]

        if(adjfac !="none") {
          colnames(scmetadata_sub)[which(colnames(scmetadata_sub) == adjfac)] <- "adjfactor"
        }
        #table(rownames(scmetadata_sub) == colnames(sccounts_sub_ct))
        ...
        if(length(tps1) >= 3) {
            #print(paste("modeling time as continuous for: ",
            #    ct, ": donorID: ", ptid, sep = ""))

            # make sca object with time as numeric continuous
            cdat <- scmetadata_sub
            #cdat$Time <- as.numeric(gsub("\\D", "", cdat$Time))
            cdat$Time <- as.numeric(gsub("[[:alpha:]]", "",cdat$Time))
            #table(rownames(cdat) == colnames(sccounts_sub_ct))
            ...
            if(adjfac == "none") {
                zlmCond <- zlm(formula = ~Time,
                       sca = sca,
                       method = "bayesglm",
                       ebayes = TRUE,
                       parallel = TRUE)
            } else {
                zlmCond <- zlm(formula = ~Time + adjfactor,
                       sca = sca,
                       method = "bayesglm",
                       ebayes = TRUE,
                       parallel = TRUE)
            ...
            lmdf <- data.frame(lmdf, donorID = ptid, celltype = ct,
                               dir = dir, stringsAsFactors = F)
            lmdf <- lmdf[order(lmdf$coef, decreasing = T),]

            # pval <- summaryDt %>%
            #         dplyr::filter(component == "H" & contrast == lrt) %>%
            #         select(primerid, contrast, `Pr(>Chisq)`)
            # coefDF <- summaryDt %>%
            #         dplyr::filter(component == "logFC" & contrast == lrt) %>%
            #         select(primerid, coef)
            # lmdf <- merge(pval, coefDF, by = "primerid") %>%
            #         dplyr::rename(nomP = `Pr(>Chisq)`) %>% mutate(adjP = p.adjust(nomP,
            #                method = "BH"), donorID = ptid, celltype = ct,
            #                dir = ifelse(coef > 0, "increasing over time",
            #                             "decreasing over time")) %>%
            #         arrange(desc(coef))

            outFile1 <- paste(filePATH,"/",fileName,"_",ptid,"_", ct,
                              "_DEGs.txt", sep = "")
            write.table(lmdf, file = outFile1, sep = "\t", quote = F,
                        row.names = F)
            ...
            colannot1 <- colannot1[colnames(mCounts2), , drop = F]
            #table(rownames(colannot1) == colnames(mCounts2))
          ...
          # downsampling to a random 50 or min cells per sample: for plotting purpose only
          set.seed(seed = 123)
          colannot2 <- cdat %>%
               rownames_to_column(var = "barcodes") %>%
               select(barcodes,
          ...
          #table(rownames(colannot2) == colnames(sccounts_sub_ct_v2))
          ...
          sccounts_sub_ct_v2 <- sccounts_sub_ct_v2[orderRow2, , drop = F]
          gaprow <- as.numeric(table(orderRow$dir)[1])

          # plot
          outFile3 <- gsub(".txt", "_scHeatmap.pdf", outFile1)
          pdf(file = outFile3, width = plotWidth, height = plotHeight)
          pheatmap(mat = sccounts_sub_ct_v2[orderRow2,
                            orderCol2],
        ...
        if(length(tps1) == 2) {
          #print(paste("modeling time as discrete for: ",
          #       ct, ": donorID: ", ptid, sep = ""))
        ...
        lrt <- paste("Time",
             ...
             sep = "")
        summaryGroup <- MAST::summary(object = zlmCond,
                      doLRT = lrt)
        ...
        #        celltype = ct,
        #        dir = ifelse(coef > 0,
        #                paste("upregulated at ",
        #               fgtp, sep = ""),
        #                paste("downregulated at ",
        #               fgtp, sep = ""))) %>%
        #         arrange(desc(coef))
        ...
        #table(rownames(colannot3) == colnames(sccounts_sub_ct_v2))
        ...
        orderRow <- lmdf %>%
            ...
            arrange(coef)
        orderRow2 <- orderRow$primerid
        sccounts_sub_ct_v2 <- sccounts_sub_ct_v2[orderRow2, ,
                         drop = F]
        gaprow <- as.numeric(table(orderRow$dir)[1])

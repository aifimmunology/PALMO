...
#' @param group_oi Features of interest such as  specific celltypes c("CD4_Naive",
#' "CD4_TEM")
...
#' #Check PALMO object <result$degs>
#' }

sclongitudinalDEG <- function(data_object, scassay="RNA",
           group_column, group_oi=NULL,
           mincellsexpressed = 0.1, removelnc = "TRUE", adjfac = NULL,
    ...
    lmLS <- lapply(ptids, function(ptid) {
      ...
      lapply(group_types, function(ct) {
        cat(paste("Fitting a ZLM model for donorID: ", ptid,
            ", within group: ", ct, sep = ""),"\n")
        ...
        if(length(tps1) >= 3) {
            #cat(paste("modeling time continuous(t=",length(tps1),"): ",ptid,"-",
            #        ct, sep = ""), "\n")
            ...
            #                method = "BH"), donorID = ptid, group_type = ct,
          ...
          # annotation colors
          ann_colors <- list(Time = c("aliceblue", #"black",
                      "yellow", "orange", "red"))
        ...
        if(length(tps1) == 2) {
          cat(paste("modeling time discrete(t=",length(tps1),"): ",ptid,"-",
                      ct, sep = ""), "\n")

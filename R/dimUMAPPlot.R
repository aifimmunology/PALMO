...
dimUMAPPlot <- function(data_object, nPC=30, gene_oi, group_column, plotname=NULL,
                        repel=FALSE, filePATH=NULL, fileName=NULL) {
  ...
  if(!is.null(data_object@curated$SeuratObj)) {
    ...
    p1 <- DimPlot(object = rnaObj, reduction = "pca", group.by = group_column, label = T, repel = repel)
    p2 <- DimPlot(object = rnaObj, reduction = 'umap', group.by = group_column, label = T, repel = repel)

    png(paste(filePATH,"/",fileName,"-UMAP-",plotname,"-Genes.png", sep=""), width=16, height=5, res=200, units = "in")
    print(plot_grid(p1, p2, align="hv"))
    dev.off()

    cat(date(),": Please check output directory for PC, UMAP plot")
    #plot result
    print(plot_grid(p1, p2, align="hv"))

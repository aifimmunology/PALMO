...
#' mentioned in https://search.r-project.org/CRAN/refmans/SeuratObject/html/CreateSeuratObject.html.
...
createPALMOfromsinglecellmatrix <- function(data, metadata, anndata=NULL) {
  ...
  data <- data[,ov]
  if(length(ov) >1) {
    cat(date(), "Number of cells overlapped=", length(ov),"\n")
    ...
    cat(date(), "Sample of matrix (column names) and annotation (row names) are
        not matching. Please check input data.\n")
  }

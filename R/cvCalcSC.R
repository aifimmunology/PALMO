...
#' Participant sample name; PTID, participant; Time, longitudinal time frame; group,
...
cvCalcSC <- function(mat=NULL, ann=NULL, meanThreshold=NULL, cvThreshold=NULL, housekeeping_genes=NULL, fileName=NULL, filePATH=NULL) {
    ...
    res <- pblapply(uniSamplegroup,function(uS) {
      ...
      thr <- round(1+max(temp[c("GAPDH","ACTB"),]$CV))
    ...
    hg_res$sample <- rn$X2
    hg_res$group <- rn$X1
    plot1 <- ggplot(hg_res, aes(x=mean, y=CV)) + geom_point() + facet_wrap(~gene)
    ...
    plot4 <- ggplot(variable_gene, aes(x=mean, y=CV)) + geom_point() +
    ...
    cat(date(),": Saving CV plots\n")
    png(paste(filePATH,"/",fileName,"-CV-distribution-HousekeepingGenes.png", sep=""), width=10, height=10, res=200, units="in")
    print(plot1)
    dev.off()

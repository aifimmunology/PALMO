...
#' @examples
#' \dontrun{
#' variancefeaturePlot(data_object=NULL, top_n=15)
#' }

variancefeaturePlot <- function(data_object=NULL, vardata=NULL,
                                featureSet="PTID",
                                Residual=FALSE, top_n=15, cols=NULL) {
  ...
  for(i in 1:length(featureList)) {
    ...
    data_sub <- data[order(data[,column_oi], decreasing = T),]
    ...
    plot <- ggplot(data_sub, aes(x=feature, y=value, fill=variable)) +
      geom_bar(stat="identity", position="stack") + labs(x="Features", y="% Variance explained", title=column_oi) +

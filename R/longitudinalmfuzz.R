...
#' @examples
#' \dontrun{
#' longitudinalmfuzz(data_object=palmo_obj)
#' }
...
longitudinalmfuzz <- function(data_object, group_column="group",
                              ...
                              max_cluster=NULL, delta=0.5, plotsize=10,
                              cl=2,
                              fileName=NULL, filePATH=NULL) {

  cat(date(),": Identifying Longitudinal Trajectories\n")
  ...
  res_group <- lapply(uniGroup, function(uIG) {
    ...
    res_donor <- lapply(uniDonor, function(uID) {
      ...
      res <- pblapply(geneList, cl=cl, function(gL) {
        ...
        suppressMessages(df1 <- df %>% group_by(time, blank) %>% summarise(val=median(gene)) %>% data.frame() %>% column_to_rownames(var="time"), classes = "message")
        df1 <- df1[uniTime_sub,]
        return(df1$val)
      })
      pboptions(op)
      ...
      temp <- data.frame(t.cl$centers, check.names = F, stringsAsFactors = F)
      ...
      temp <- data.frame(temp, trajectory_direction=temp1, cluster=row.names(temp), check.names = F, stringsAsFactors = F)

      #Cluster information
      temp$group <- uIG
      temp$donor <- uID
      write.csv(temp, file = paste(filePATH,"/longitudinal_",fileName,"_",uID, "_", uIG, ".csv", sep=""), quote = F)

      plot_panels <- ceiling(sqrt(nCluster))
      pdf(paste(filePATH,"/longitudinal_",fileName,"_",uID, "_", uIG, ".pdf", sep=""), width = plotsize, height = plotsize)
      mfuzz.plot(expDF,cl=t.cl,time.labels=labels,mfrow=c(plot_panels,plot_panels), new.window=F)
      dev.off()

      mRes <- data.frame(Genes=names(t.cl[["cluster"]]), cluster=t.cl[["cluster"]], stringsAsFactors = F)
      write.csv(mRes, file = paste(filePATH,"/longitudinal_",fileName,"_",uID, "_", uIG, "_geneclustering.csv", sep=""), quote = F)
      mRes <- mRes[geneList,]

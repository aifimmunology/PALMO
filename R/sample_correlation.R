...
sample_correlation <- function(data_object, donor_sep = ":",
                               cor_method = "spearman", group_position = 2,
                               group_by = "donor", col_break = NULL,
                               col_max = 1, column_names_fontsize = 4,
                               row_names_fontsize = 4,
                               row_title_fontsize = 6,
                               column_title_fontsize = 6, plotHeight = 20,
    ...
    temp <- data.frame(do.call(rbind, strsplit(data_annotation$id,
                            split = donor_sep)), stringsAsFactors = FALSE)

    if (ncol(temp) > 1) {
        data_annotation <- data.frame(data_annotation, Sample = temp$X1,
                        group = temp$X2, stringsAsFactors = FALSE)
        ...
        ht1 <- Heatmap(data.matrix(res),
                       cluster_rows = FALSE, cluster_columns = FALSE,
                       na_col = "grey",
                       col = colorRamp2(c(-1, -0.9, 0, 0.9, 1),
                            c("blue", "skyblue", "white", "pink", "red")),
                       row_names_max_width = unit(10, "cm"),
                       column_names_gp = gpar(fontsize = column_names_fontsize),
                       row_names_gp = gpar(fontsize = row_names_fontsize),

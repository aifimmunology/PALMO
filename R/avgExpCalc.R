...
#' function AverageExpression (https://satijalab.org/seurat/reference/averageexpression)
...
avgExpCalc <- function(data_object, assay="RNA", group_column) {
  ...
  if(!is.null(data_object@curated$SeuratObj)) {
    ...
    scrna_avgmat <- AverageExpression(object=dataObj, assays=assay,
                                      slot="counts", group.by="Sample_group",
                                      verbose=TRUE)
    cn <- data.frame(colnames(scrna_avgmat[[assay]]))
    cn <- gsub("data\\[, 1]", "", as.character(row.names(cn)))
    scrna_avgmat <- data.frame(scrna_avgmat[[assay]], check.names = F,
                               stringsAsFactors = F)
    ...
    cat(date(),": Keeping genes with avergae expression >0 in all samples\n")

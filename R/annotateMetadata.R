...
annotateMetadata <- function(data_object, sample_column="Sample",
  ...
  if(sum(is.na(anndata$Sample))>0 | sum(is.na(anndata$PTID))>0 |
    ...
    cat(date(),": Caution -> The Sample_column, Donor_column or Time_column
        contains missing value or NA. Missing data is removed.\n")
  ...
  row.names(anndata) <- make.names(anndata$Sample, unique = T)

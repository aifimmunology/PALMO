...
gene_featureplot <- function(data_object=NULL, data=NULL, anndata=NULL, featureList,
                             x_group_by="PTID", var_oi="Time", xlab="group_by",
                             ylab="Value/Expression", ncol=2, facet_by=NULL,
                             compare_means=FALSE,
  ...
  for(i in 1:length(featureList)) {
    ...
    df <- data.frame(exp=as.numeric(mat[featureName,]), ann, stringsAsFactors = F)
    ...
    p1 <- ggplot(df, aes(x=group_oi, y=exp)) +
      geom_boxplot(outlier.shape = NA) +
      labs(x=xlab, y=ylab, title=featureName) +
      geom_jitter(width=0.2, aes(shape=var_oi)) +
      scale_shape_manual(values=1:nlevels(df$var_oi)) +
    ...
    if(!is.null(x_text_angle)) {
      p1 <- p1 + theme(axis.text.x = element_text(angle=90, hjust = 0.5, vjust = 1),legend.position = "right")
    }

...
#' @param data_object Input \emph{PALMO} S4 object. Contains annotation table and expression
#' matrix or data frame. Rows represent gene/proteins column represents
#' participant samples (same as annotation table Sample column)
...
checkReplicates <- function(data_object, mergeReplicates=FALSE) {
  ...
  rep <- rep[rep$Freq>1,]
  if(nrow(rep)>0) {
    cat(date(),": Replicates for data found\n")
    print(rep)
    ...
    if(mergeReplicates == TRUE) {
      cat(date(),": Merging replicates by Median\n")
      ...
      for(i in 1:length(PTID_Time)) {
        ...
        if(length(group_info) ==0) {
          ...
          ann[ann$PTID_Time %in% pt,]$Sample_group <- paste(pt, ann[ann$PTID_Time %in% pt,]$group, sep=":")
        }
      }
      ...
      med_agg_res <- lapply(uni_sample_group, function(sg) {
        #sg <- uni_sample_group[1]
        #print(sg)
        ann_sub <- ann[ann$Sample_group %in% sg,]
        mat_sub <- mat[,row.names(ann_sub)]
        if(nrow(ann_sub)>1) {
          mat_agg <- apply(mat_sub, 1, function(x) { median(x, na.rm=T)})
        } else {
          mat_agg <- mat_sub
        }
        mat_agg <- data.frame(mat_agg, check.names = F, stringsAsFactors = F)
        colnames(mat_agg) <- sg
        return(mat_agg)
      })
      med_agg_res <- do.call(cbind, med_agg_res)
      ## Aggregated result
      med_agg_res <- data.frame(med_agg_res, check.names = F, stringsAsFactors = F)
      ...
      cat(date(),": Check PALMO object (anndata, data). To ignore replicates use mrgereplicates=FALSE.\n")
    } else {
      cat(date(),": Merging Replicates ignored\n")
    }
  } else {
    cat(date(),": No Replicates found\n")
  }

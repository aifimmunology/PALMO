...
#' @param selectedFeatures Optional, focus on selected genes/features. Default NULL
#' @param median_cv_max Optional, Remove features with greater than median_cv_max.
#' Default is NULL
...
cvCalcBulk <- function(data_object, meanThreshold=NULL,
    ...
    if(is.null(median_cvThreshold)) {
      ...
      cat(date(),": Using median CV threshold (across donors) same as CV
    ...
    if(!is.null(selectedFeatures)) {
      cat(date(),": Filtering for selected features", "\n")
      ...
      if(nrow(meta_df)>1) {
        ...
        if(nrow(dp2a)>0) {
          ...
          variable_gene <- rbind(variable_gene,
                                 data.frame(donor=uS, feature=row.names(dp2a),
                                            dp2a[,c("mean","sd","var","CV", "NAs", "Zeros")],
                                            stringsAsFactors = F))
        }
        ...
        if(nrow(dp2b)>0) {
          ...
          stable_gene <- rbind(stable_gene,
                               data.frame(donor=uS, feature=row.names(dp2b),
                                          dp2b[,c("mean","sd","var","CV", "NAs", "Zeros")],
                                          stringsAsFactors = F))
        ...
        temp <- data.frame(feature=row.names(df), var=df$CV, stringsAsFactors = F)
        colnames(temp)[-1] <- paste(uS, "_", colnames(temp)[-1], sep="")
        if(i==1) {
          res <- temp
        } else {
          res <- merge(res, temp, by="feature", all=T)
        }
      }
    ...
    if(ncol(res) == 2) {
      res_var <- data.frame(res[,-1], stringsAsFactors = F)
      ...
      max_cv <- max(abs(res_var$Median), na.rm=T)
      if(median_cvThreshold> max_cv) {
        median_cvThreshold <- max_cv
        cat(date(),": Input median_cv_max higher than maximum CV range.\n")
      }
      ...
      #color list
      if(min(res_var$Median,na.rm=T)<0) {
        col_list <- colorRamp2(c(-max_cv, -cvThreshold-0.01, -cvThreshold, -cvThreshold/2, 0, cvThreshold/2, cvThreshold, cvThreshold+0.01, max_cv),
                               c("red", "pink", "blue", "skyblue", "black", "skyblue", "blue", "pink", "red"))
      } else {
        col_list <- colorRamp2(c(0, cvThreshold/2, cvThreshold, cvThreshold+0.01, max_cv),
                               c("black", "skyblue", "blue", "pink", "red"))
      }

      ## Plot variable genes CV
      mat <- res_var[order(-(res_var$NAs), abs(res_var$Median), decreasing = T),]
      ...
      if(nrow(mat)>1) {
        ...
        check_0 <- unique(as.numeric(unlist(apply(column_mat,2,function(x) { unique(x,na.rm=T) }) )))
        if(length(check_0) != 1) {
        ...
        print(ht1)
        pdf(paste(filePATH,"/",fileName,"-CV-Variable-Heatplot.pdf", sep=""),
            width=plotWidth, height=plotHeight)
        draw(ht1)
        dev.off()
      } else {
        var_mat <- NA
        cat(date(),": Variable features do not found. Check data for missing
          values or change CV parameters\n")
      }

      ## Plot stable genes CV
      mat <- res_var[order(res_var$NAs, abs(res_var$Median), decreasing = F),]
      mat <- mat[abs(mat$Median) <= median_cvThreshold,]
      if(nrow(mat)>1) {
        stable_mat <- mat
        mat <- mat[,uniSample]
        ...
        stable_mat <- NA
        cat(date(),": Stable features do not found. Check data for missing
          values or change CV parameters. Summary of CV range in given data:\n")
        print(summary(res_var$Median))
      }
    ...
    cat(date(),": Done. Please check output directory for Plots/results.\n")
    return(data_object)
}

...
lmeVariance <- function(data_object, featureSet,
                        fixed_effect_var=NULL, meanThreshold=NULL,
                        selectedFeatures=NULL, NA_to_zero=FALSE,
                        cl=2,
    ...
    suppressMessages(lmem_res <- pblapply(1:length(rowN), cl=cl, function(gn) {
      ...
      if(check_gene_group == 0) {
        ...
        return(c(geneName, mean(df$exp, na.rm=T), median(df$exp, na.rm=T),
                 sd(df$exp, na.rm=T), max(df$exp, na.rm=T),
                 (lmem_re$vcov)/sum(lmem_re$vcov)) )
      }
    }) , classes = "message")
    pboptions(op)
    lmem_res <- do.call(rbind, lmem_res)
    lmem_res <- data.frame(lmem_res, check.names=F, stringsAsFactors = F)
    ...
    temp <- apply(lmem_res[,-1],1, function(x) { as.numeric(x)})
    row.names(temp) <- colnames(lmem_res)[-1]
    lmem_res <- data.frame(Gene=colnames(temp), t(temp), check.names=F, stringsAsFactors = F)
    ...
    for(i in 1:length(featureSet)) {
      dfx <- res[order(res[,featureSet[i]], decreasing = T),]
